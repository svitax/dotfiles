...
(use-package capf
  ...
  (when (fboundp 'cape-capf-prefix-length)
    (defalias '+capf-general (cape-capf-prefix-length
                              (cape-capf-super
                               #'cape-dabbrev ; `dabbrev-capf' errors
                               #'cape-dict ; `ispell-complete-word' also?
                               #'cape-keyword
                               #'cape-line)
                              1))
    (defalias '+capf-file (cape-capf-prefix-length #'cape-file 1))
    (defalias '+capf-template (cape-capf-prefix-length #'tempel-expand 1))

    (defun +capf-add-capfs (&optional global)
      "Add `completion-at-point' functions to GLOBAL hook if non-nil, else
local."
      (let ((local (not global)))
        (add-hook 'completion-at-point-functions #'+capf-general -10 local)
        (add-hook 'completion-at-point-functions #'+capf-template -10 local)
        (add-hook 'completion-at-point-functions #'+capf-file -10 local)
        (when local
...
;; TODO document org-repeat-todo
(use-package org-repeat-todo
  :no-require
  :config
